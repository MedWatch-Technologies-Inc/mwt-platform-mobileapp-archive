...
import 'map_screen/providers/location_track_provider.dart';

class BPMonitoringScreen extends StatefulWidget {
  @override
  _BPMonitoringScreenState createState() => _BPMonitoringScreenState();
}

class _BPMonitoringScreenState extends State<BPMonitoringScreen> {
      ...
      appBar: PreferredSize(
          ...
          child: Container(
            ...
            child: AppBar(
              elevation: 0,
              leading: IconButton(
                key: Key('clickOnBackButtonBPMonitoring'),
              ...
              centerTitle: true,
              title: Text(
                StringLocalization.of(context).getText(StringLocalization.bpMonitoring),
                style: TextStyle(
  ...
  Widget BPMonitorSwitch() {
    return ListTile(
        title: Text(StringLocalization.of(context).getText(StringLocalization.bpMonitoring)),
        trailing: Padding(
          padding: EdgeInsets.zero,
          key: Key('clickOnBPMonitoringToggle'),
          child: ValueListenableBuilder(
              valueListenable: _helper.deviceSettingNotifier,
              builder: (context, value, widget) {
                return CustomSwitch(
                  value: value?.bpMonitoring.status ?? true,
                  onChanged: (value) {
                    _helper.deviceSetting?.bpMonitoring.status = value;
                    _helper.deviceSettingNotifier.notifyListeners();
                    setMonitorInDevice();
                    preferences?.setBool(Constants.isBPMonitorOnKey, value);
                    setState(() {});
                  },
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, value, widget) {
        return ListTile(
          onTap: () {
            // int selectedIndex =
            //     timeIntervalList.indexOf(value?.bpMonitoring.timeInterval.toInt() ?? 5);
            // if (selectedIndex > -1) {
            //   timeIntervalController = FixedExtentScrollController(
          ...
          trailing: Row(
            ...
            children: <Widget>[
              Text('${value?.bpMonitoring.timeInterval.toInt() ?? 5} min'),
          ...
          Container(
            ...
            children: [
              FlatBtn(
                  onPressed: () {
                    try {
                      _helper.deviceSetting?.bpMonitoring.timeInterval =
  ...
  void setMonitorInDevice() {
    try {
      preferences?.setInt(
          Constants.bpRateInterval, _helper.deviceSetting?.bpMonitoring.timeInterval.toInt() ?? 5);
      connections.setBPMonitorOn(
        _helper.deviceSetting?.bpMonitoring.status ?? true,
        _helper.deviceSetting?.bpMonitoring.timeInterval.toInt() ?? 5,
        context: context,
        showLoader: true,
      );
    } catch (e) {
