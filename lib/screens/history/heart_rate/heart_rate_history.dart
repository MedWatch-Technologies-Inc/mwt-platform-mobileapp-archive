...
import 'app_bar.dart';
import 'heart_rate_day_tab.dart';
import 'heart_rate_month_tab.dart';
import 'heart_rate_week_tab.dart';
...
class _HeartRateHistoryState extends State<HeartRateHistory>
  ...
  late HeartRateHistoryProvider provider;
  late HeartRateDayDataProvider dayDataProvider;
  late HeartRateWeekDataProvider weekDataProvider;
  late HeartRateMonthDataProvider monthDataProvider;

  @override
  void initState() {
    Future.delayed(Duration.zero).then((value) {
      provider = Provider.of<HeartRateHistoryProvider>(context, listen: false);
      dayDataProvider =
          Provider.of<HeartRateDayDataProvider>(context, listen: false);
      weekDataProvider =
          Provider.of<HeartRateWeekDataProvider>(context, listen: false);
      monthDataProvider =
    ...
    return SafeArea(
      ...
      child: Scaffold(
        ...
        body: Column(
          ...
          children: [
            Consumer<HeartRateHistoryProvider>(
              builder: (BuildContext context, HeartRateHistoryProvider provider,
                  Widget? child) {
                return DayWeekMonthTab(
                  ...
                  onTapNextDate: () {
                    var isEnable = true;
                    var diff = provider.selectedDate.difference(DateTime.now());
                  ...
                  onTapCalendar: () async {
                    var date = await Date(getDatabaseDataFrom: 'Heart Rate')
            ...
            Expanded(
              child: PageView(
                ...
                onPageChanged: (int page) {
                  var provider = Provider.of<HeartRateHistoryProvider>(context,
                      listen: false);
  ...
  String selectedValueText() {
    ...
    if (provider.currentIndex == 0) {
      ...
      var last = DateFormat(DateUtil.ddMMyyyy).format(provider.lastDateOfWeek);
      return '$first   to   $last';
    } else if (provider.currentIndex == 2) {
      var date = DateFormat(DateUtil.MMMMyyyy).format(provider.selectedDate);
