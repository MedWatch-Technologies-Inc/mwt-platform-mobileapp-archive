...
class TemperatureMonitoringScreen extends StatefulWidget {
  @override
  _TemperatureMonitoringScreenState createState() => _TemperatureMonitoringScreenState();
}
...
class _TemperatureMonitoringScreenState extends State<TemperatureMonitoringScreen> {
      ...
      appBar: PreferredSize(
        ...
        child: Container(
          ...
          child: AppBar(
            ...
            centerTitle: true,
            title: Text(
              StringLocalization.of(context).getText(StringLocalization.temperatureMonitoring),
              style: TextStyle(
  ...
  Widget temperatureMonitorSwitch() {
    return ListTile(
        title:
            Text(StringLocalization.of(context).getText(StringLocalization.temperatureMonitoring)),
        trailing: Padding(
          padding: EdgeInsets.zero,
          key: Key('clickOnTempToggle'),
          child: ValueListenableBuilder(
              valueListenable: _helper.deviceSettingNotifier,
              builder: (context, value, widget) {
                return CustomSwitch(
                  value: value?.temperatureMonitoring.status ?? true,
                  onChanged: (value) {
                    _helper.deviceSetting?.temperatureMonitoring.status = value;
                    _helper.deviceSettingNotifier.notifyListeners();
                    if (connections != null) {
                      connections.setTemperatureMonitorOn(
                          _helper.deviceSetting?.temperatureMonitoring.status ?? true,
                          _helper.deviceSetting?.temperatureMonitoring.timeInterval.toInt() ?? 5);
                    }
                    preferences?.setBool(Constants.isTemperatureMonitorOnKey, value);
                    setState(() {});
                  },
    ...
    return ListTile(
      onTap: () {
        int selectedIndex = timeIntervalList
            .indexOf(_helper.deviceSetting?.temperatureMonitoring.timeInterval.toInt());
        if (selectedIndex > -1) {
          timeIntervalController = FixedExtentScrollController(
      ...
      trailing: Row(
        ...
        children: <Widget>[
          Text('${_helper.deviceSetting?.temperatureMonitoring.timeInterval.toInt()} min'),
          ...
          Container(
            ...
            children: [
              ...
              FlatBtn(
                  onPressed: () {
                    try {
                      _helper.deviceSetting?.temperatureMonitoring.timeInterval =
                          timeIntervalList.elementAt(timeIntervalController.selectedItem);
                      connections.setTemperatureMonitorOn(
                          _helper.deviceSetting?.temperatureMonitoring.status ?? true, _helper.deviceSetting?.temperatureMonitoring.timeInterval.toInt() ?? 5);
                      preferences?.setInt(Constants.temperatureTimeInterval, _helper.deviceSetting?.temperatureMonitoring.timeInterval.toInt() ?? 5);
                      if (this.mounted) {
                        Navigator.of(context, rootNavigator: true).pop();
                        setState(() {});
                      }
